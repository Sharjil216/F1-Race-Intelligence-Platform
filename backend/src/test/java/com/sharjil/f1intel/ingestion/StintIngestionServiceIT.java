...
class StintIngestionServiceIT {
    ...
    void stubStintsEndpoint() throws Exception {
        String body = new ClassPathResource("fixtures/stints-9558.json")
                .getContentAsString(StandardCharsets.UTF_8);

        wireMock.stubFor(get(urlPathEqualTo("/v1/stints"))
    ...
    void ingestsStintsAndIsIdempotent() {
        ...
        assertEquals(EXPECTED_STINTS, countStints());

        stintIngestionService.ingestStints(9558);
        assertEquals(EXPECTED_STINTS, countStints());
    }

    private long countStints() {
        return jdbcClient.sql("SELECT COUNT(*) FROM stint")
                .query(Long.class)
                .single();
    }
}